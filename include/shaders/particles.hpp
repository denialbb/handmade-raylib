...
struct ParticleExplosion {
    ...
    void render(Vector2 pos) {
        ...
        SetShaderValue(shader, loc_color_tint, &color_tint,
                       SHADER_UNIFORM_VEC3);

        // Draw
        Rectangle dest = {pos.x, pos.y, explosion_source_rect.width,
                          explosion_source_rect.height};
        Vector2 origin = {0, 0};

        BeginShaderMode(shader);
        DrawTexturePro(explosion_tex, explosion_source_rect, dest, origin, 0.0f,
                       WHITE);
        EndShaderMode();
    }
